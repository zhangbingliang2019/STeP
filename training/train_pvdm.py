...
def train_pvdm(cfg):
    ...
    idm, optimizer, image_dataloader, video_dataloader = accelerator.prepare(idm, optimizer, image_dataloader, video_dataloader)
    ...
    for epoch in range(training_cfg.num_epochs):
        ...
        for video in progress_bar:
            ...
            if np.random.rand() < training_cfg.joint_ratio:
                ...
                with torch.no_grad():
                    latents = images.view(batch_size, frame_size, *images.shape[1:])
                noise = torch.randn_like(latents)
                timesteps = torch.randint(0, noise_scheduler.config.num_train_timesteps, (latents.shape[0],), device=device, dtype=torch.int64)
                weights = get_loss_weight(timesteps, noise_scheduler.config.num_train_timesteps, weight_type='cosine')
                noisy_latents = noise_scheduler.add_noise(latents, noise, timesteps)
        ...
        if accelerator.is_main_process and (epoch + 1) % training_cfg.log_interval == 0:
            sampling_model = SpatialTemporalPixelDiffusionModel(
                accelerator.unwrap_model(idm), noise_scheduler, video_shape=(frame_size, image_channels, image_size, image_size),
            )
